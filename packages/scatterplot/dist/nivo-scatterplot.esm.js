...
import { noop, withTheme, withDimensions, withMotion, getAccessorOrValue, Container, CartesianMarkers, SvgWrapper, ResponsiveWrapper, getRelativeCursor, isCursorInRect } from '@nivo/core';
...
import { computeMeshPoints, computeMesh, Mesh, renderVoronoiToCanvas, renderVoronoiCellToCanvas } from '@nivo/voronoi';
...
var commonEnhancers = [withTheme(), withDimensions(), withMotion(), withPropsOnChange(['colors'], function (_ref) {
  ...
  var data = _ref3.data,
      ...
      width = _ref3.width,
      height = _ref3.height;
  var computedData = computeXYScalesForSeries(data, xScale, yScale, width, height);
...
var enhanceCanvas = function enhanceCanvas(Component) {
  return compose.apply(void 0, [defaultProps(_objectSpread$1({}, ScatterPlotDefaultProps, {
    ...
    var points2d = computeMeshPoints({
      points: points
    });
    return computeMesh({
      points: points2d,
...
function (_Component) {
  ...
  function ScatterPlot() {
    ...
    _defineProperty$3(_assertThisInitialized(_this), "showTooltip", function (showTooltip, point, event) {
      ...
      showTooltip(React.createElement(ScatterPlotTooltip, {
        point: point,
  ...
  _createClass(ScatterPlot, [{
    ...
    value: function render() {
      ...
      return React.createElement(Container, {
        ...
        if (animate === true) {
          layerById.points = React.createElement(TransitionMotion, {
            key: "points",
            ...
            return React.createElement("g", null, interpolatedStyles.map(function (_ref2) {
              var key = _ref2.key,
                  style = _ref2.style,
                  point = _ref2.data;
              return React.createElement(ScatterPlotItem$1, {
                key: key,
                point: point,
                x: style.x,
                y: style.y,
...
var findNodeUnderCursor = function findNodeUnderCursor(nodes, margin, x, y) {
  return nodes.find(function (node) {
    return isCursorInRect(node.x + margin.left - node.size / 2, node.y + margin.top - node.size / 2, node.size, node.size, x, y);
  });
};
...
export { ResponsiveScatterPlot, ResponsiveScatterPlotCanvas, ScatterPlot$1 as ScatterPlot, ScatterPlotCanvas$1 as ScatterPlotCanvas, ScatterPlotDefaultProps, ScatterPlotPropTypes };
